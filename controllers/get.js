const { nok_api } = require('../db_api/get_details');

const fs = require("fs");
const getGMSDetails = async (req, res) => {

    const userdata = req.query['get_ID'] ;
    ...
    try {
        ...
        context.get_ID = userdata;
        const dataInfo = await nok_api.GetGmsData(context);
        // console.log(dataInfo);
        if (dataInfo.length) {
            const data = {};
            data['ID'] = dataInfo[0]['ID'] ? dataInfo[0]['ID'] : "";
            data['GML_CODE'] = dataInfo[0]['GML_CODE'] ? dataInfo[0]['GML_CODE'] : "";
            data['GML_DESCP'] = dataInfo[0]['GML_DESCP'] ? dataInfo[0]['GML_DESCP'] : "";
